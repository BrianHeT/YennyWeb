<x-layout>
    <x-slot:title>Editar Noticia: {{ $article->title }}</x-slot:title>

    <h1 class="mb-3">Editar Noticia: {{ $article->title }}</h1>
    @if ($errors->any())
    ...
    <form action="{{ route('articles.update', ['id' => $article->article_id]) }}" method="post">
        @csrf
        @method('PUT')
        <div class="mb-3">
